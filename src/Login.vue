...
onMounted(() => {
  ...
  if (window.google && window.google.accounts && window.google.accounts.id) {
    window.google.accounts.id.initialize({
      client_id:
        "957915397315-heejc9v3hqv5vbg2r4ao2lo15snaaor1.apps.googleusercontent.com",
...
const handleLoginSuccess = (response) => {
  ...
  try {
    ...
    if (allowedEmails.includes(decoded.email)) {
      const userWithToken = {
        ...decoded,
        token: credential, // ✅ Aquí se incluye el token
        model: "", // puedes ajustar esto según tu lógica
      };
